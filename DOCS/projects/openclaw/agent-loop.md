...
- 核心源码：
  - Session 外壳：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts)
  - Agent 核心：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts)
  - Loop 本体：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts)
  - 类型定义：[types.ts](../../../openclaw/packages/agent-core/src/types.ts)
...
[AgentSession](../../../openclaw/src/agents/sessions/agent-session.ts) 是面向产品运行时的会话对象。它不是模型/工具循环本体，而是负责把核心 Agent loop 接到 OpenClaw 的会话体系里。
...
- `AgentSession` class：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts#L334)
- `installAgentToolHooks(...)`：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts#L497)
- `handleAgentEvent(...)`：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts#L576)
- `runAgentPrompt(...)`：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts#L1089)
- `prompt(...)`：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts#L1133)

### Agent：状态、事件与消息队列

[Agent](../../../openclaw/packages/agent-core/src/agent.ts) 是核心 Agent 抽象，负责维护 `AgentState`、订阅 / 处理 `AgentEvent`，并暴露 `prompt()`、`continue()`、`steer()`、`followUp()` 等方法。

关键入口：

- `Agent` class：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L203)
- `steer(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L313)
- `followUp(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L318)
- `prompt(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L374)
- `continue(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L390)
- `runPromptMessages(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L438)
- `processEvents(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L568)

### runLoop：模型 / 工具循环本体

真正执行“模型调用 -> 工具执行 -> 工具结果回写 -> 再调用模型”的核心逻辑在 [agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts)。

关键入口：

- `runAgentLoop(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L171)
- `runAgentLoopContinue(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L198)
- `runLoop(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L265)
- `streamAssistantResponse(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L446)
- `executeToolCalls(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L548)

所以如果问“OpenClaw 的 Agent Loop 在哪里”，可以分两层回答：

1. **核心循环本体**在 [agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts)，尤其是 `runLoop(...)`。
2. **产品化 session runtime**在 [agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts)，负责把核心 loop 接到会话、工具 hook、压缩、持久化和 UI 事件。
...
这部分对应 [runLoop(...)](../../../openclaw/packages/agent-core/src/agent-loop.ts#L265) 中围绕 assistant response、tool calls 和 tool execution 的循环，以及 [executeToolCalls(...)](../../../openclaw/packages/agent-core/src/agent-loop.ts#L548)。
...
- `Agent.steer(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L313)
- `runLoop(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L265)
...
- `Agent.followUp(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L318)
- `runLoop(...)`：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L265)
...
- `processEvents(...)`：[agent.ts](../../../openclaw/packages/agent-core/src/agent.ts#L568)
- loop event 产生：[agent-loop.ts](../../../openclaw/packages/agent-core/src/agent-loop.ts#L265)
- Session event 处理：[agent-session.ts](../../../openclaw/src/agents/sessions/agent-session.ts#L576)
...
OpenClaw 则自己手写 [runLoop(...)](../../../openclaw/packages/agent-core/src/agent-loop.ts#L265)，并在 core Agent 中放入 steer/followUp 队列，因此更像：
