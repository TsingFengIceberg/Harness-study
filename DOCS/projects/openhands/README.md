...
- **当前快照**:
  - `openhands`: `2abf0b1303c8d3c8dcd9fb080507395afbaa21d4`
  - `software-agent-sdk`: `65ee52f817f5b30292abe5f2ac620f84576eb91f`
...
| [context-management.md](context-management.md) | draft | OpenHands Context Management：控制面准备上下文材料，执行面以 ConversationState / EventLog / View / Condensation 管理事件账本、当前分支投影与 LLM messages。 |
| [permission-security.md](permission-security.md) | draft | OpenHands Permission / Security：ActionEvent 风险评估、SecurityAnalyzer、ConfirmationPolicy、WAITING_FOR_CONFIRMATION、UserRejectObservation 与 workspace / sandbox 边界。 |
| [sandbox-workspace.md](sandbox-workspace.md) | draft | OpenHands Sandbox / Workspace：App Server 控制面、sandbox 内 Agent Server、RemoteWorkspace / LocalWorkspace 视角差异、Terminal / FileEditor 执行落点与 workspace 生命周期。 |
| [multi-agent.md](multi-agent.md) | draft | OpenHands Multi-Agent：控制面 `enable_sub_agents`、执行面 `TaskToolSet`、`AgentDefinition` registry、TaskObservation 与前端 sub-agent card。 |
...
| `sandbox-workspace.md` | App Server 控制面如何创建 / 管理 sandbox，sandbox 内 Agent Server 如何把 Terminal / FileEditor 落到 workspace 执行 | draft |
| `tool-system.md` | terminal、file editor、task tracker、browser、subagent、MCP / ACP 工具体系 | draft |
| `multi-agent.md` | `TaskToolSet`、`AgentDefinition`、内置 / 项目 / 用户 / 插件 sub-agent、前端 sub-agent task card | draft |
| `context-management.md` | ConversationState、EventLog、View、Condensation、AgentContext、conversation tree / branch 与 LLM messages 投影 | draft |
| `permission-security.md` | ActionEvent risk、ConfirmationPolicy、SecurityAnalyzer、UserRejectObservation、workspace / sandbox 边界 | draft |
| `conversation-tree.md` | conversation tree / branch、fork / navigate、active branch、轨迹回溯与 UI 展示 | planned |
