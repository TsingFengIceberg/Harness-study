...
> **日期**: 2026-07-01 | **状态**: draft | **涉及版本**: `openhands` submodule `c105a8238`
...
- 外部执行面源码：OpenHands 当前仓库说明 Agent / Agent Server 源码已迁至 [OpenHands/software-agent-sdk](https://github.com/OpenHands/software-agent-sdk)，本文涉及 `Conversation.run()`、`Agent.step()`、`ActionEvent` / `ObservationEvent` 的判断来自该外部仓库少量源码片段核验。
- 横向 QA：[Harness Study 横向 QA](../../comparison/qa.md#agent-loop--控制流)

## 1. 核心结论

OpenHands 的 Agent Loop 不能只在当前 `OpenHands/OpenHands` 仓库里找一个类似 `run_turn` 的函数。当前仓库更像 **Agent Canvas / App Server 控制面**，真正的 Agent / Agent Server / SDK loop 已迁到外部 [OpenHands/software-agent-sdk](https://github.com/OpenHands/software-agent-sdk)。这一点在 OpenHands 当前仓库 [README.md](../../../openhands/README.md#L49-L54) 中有明确说明。
...
- OpenHands Agent 和 Agent Server 源码在 [OpenHands/software-agent-sdk](https://github.com/OpenHands/software-agent-sdk)；
...
| 外部路径 | 作用 |
|---|---|
| `openhands-sdk/openhands/sdk/conversation/impl/local_conversation.py` | `LocalConversation.run()` / `arun()`，外层 conversation execution loop。 |
| `openhands-sdk/openhands/sdk/agent/agent.py` | `Agent.step()` / `astep()`，单步 LLM 决策和 action 执行。 |
| `openhands-sdk/openhands/sdk/agent/response_dispatch.py` | 将 LLM response 分类为 `TOOL_CALLS` / `CONTENT` / `REASONING_ONLY` / `EMPTY` 并分派。 |
| `openhands-sdk/openhands/sdk/event/llm_convertible/action.py` | Action 事件模型。 |
| `openhands-sdk/openhands/sdk/event/llm_convertible/observation.py` | Observation 事件模型。 |
| `openhands-agent-server/openhands/agent_server/conversation_router.py` | Agent Server conversation REST API，例如 `/api/conversations/{id}/run`。 |
| `openhands-agent-server/openhands/agent_server/event_router.py` | Agent Server event REST API，例如 `/api/conversations/{id}/events`。 |

> 注意：这些文件不在当前 submodule 内，正式写横向专题时可以考虑把 `software-agent-sdk` 作为参考仓库单独纳入，或在脚注里明确标注“外部官方仓库源码核验”。
...
A: 当前 `OpenHands/OpenHands` 仓库主要是 Agent Canvas / App Server 控制面，真正的 OpenHands Agent 和 Agent Server 源码已经迁到外部 [OpenHands/software-agent-sdk](https://github.com/OpenHands/software-agent-sdk)。当前仓库负责启动 / 管理 sandbox、构造 conversation request、转发用户消息、查询状态和存储事件；真正的 `Conversation.run()` / `Agent.step()` / ActionEvent / ObservationEvent loop 在外部 SDK / Agent Server 中。
