...
> **日期**: 2026-07-10 | **状态**: draft | **��及版本**: `c9fb9768d476e28de0294ac7a23cab9819b93f83`
>
> **2026-07-13 更新**: 基于 `42544755` 快照补充 Memory Tool Mode 和 Memory Consolidation。
...
### 2.3 Memory Tool Mode：Agent 主动管理自己的长期记忆（v2.0+ 新增）

这是 DeerFlow Memory 本轮最大变化。之前只��� **middleware mode**（默认）：`MemoryMiddleware` 在 Agent 完成后自动筛选对话、异步排队、由 LLM 提取 facts，Agent 自己不主动操作记忆。

新版本新增了 **tool mode**（`memory.mode: "tool"`），核心实现在 [memory/tools.py](../../../submodules/deer-flow/backend/packages/harness/deerflow/agents/memory/tools.py)。Agent 可以主动调用四个 memory 工具：

| 工具 | ���用 | 示例用法 |
|---|---|---|
| `memory_search` | 按自然语言搜索已有 facts | "查一下用户的偏好" |
| `memory_add` | 创建新 fact，含 content/category/confidence | "记住用户是 Go 开发者" |
| `memory_update` | 修改已有 fact 的内容/分类/置信度 | "更新上次的偏���" |
| `memory_delete` | 删除不再准确的 fact | "这条已经过时了，删掉" |

模式由 [memory_config.py](../../../submodules/deer-flow/backend/packages/harness/deerflow/config/memory_config.py) 的 `mode` 字段控制：

```yaml
mode: "middleware"  # 默��：被动背景注入，Agent 不操作
mode: "tool"        # 新增：Agent 主动用 memory_add/search/update/delete
```

两种模式**互斥**——`MemoryMiddleware` 不会在 tool mode 下运行。

这一变化的重要含义：

```text
middleware mode:
  用户对话 -> 中间件自动提取 -> 写入 memory -> 下次注入 prompt
  Agent 不知道自己"在记忆"，也不知道记忆了什么

tool mode:
  Agent 决定何时记忆、记什么、何时搜索记忆、何时删���
  Memory 从"被动背景机制"升级为"Agent 主动���作的能力"
```

**精��标记：** tool mode 让 DeerFlow 从"自动替你记"变成"Agent 自己决定记什么"——这和 Anthropic 的 Memory Tool（`memory_20250818`）方向一致。

### 2.4 Memory Consolidation：LLM 驱动的记忆碎片整理

另一个新增功能是 Memory Consolidation。当某个 category 下的 facts 数量超过 `consolidation_min_facts`（默认 8），同一轮 MemoryUpdater 调用中 LLM 会判断哪些碎片化 facts 可以合并成一条综合 fact。

```text
合并���：5 条零散的"用户偏好 Python 类型标注"事实
合并后：1 条综合 fact，"合并自 N 条来源"，置信度不超过源最大值
```

候选选择和受限合并实现在 [updater.py](../../../submodules/deer-flow/backend/packages/harness/deerflow/agents/memory/updater.py) 的 `_select_consolidation_candidates` 和 `_apply_updates` normalization 部分，默认由 `consolidation_enabled: true` 控制���

**局限：** Consolidation 只做碎片合并，不做矛盾检测���语义冲突解决。多源 fact 之间���逻辑矛盾仍然需要 stale review 或人工处理。

### 2.5 安全加固

本轮多个 commit 对 memory 内容在���入 prompt 前做 HTML-escape，防��� memory 中��攻击性内���通过 prompt injection 进入系统提示。详见 [updater.py](../../../submodules/deer-flow/backend/packages/harness/deerflow/agents/memory/updater.py) 和 [memory/prompt.py](../../../submodules/deer-flow/backend/packages/harness/deerflow/agents/memory/prompt.py) 相关 commit（#4028、#4044、#4097、#4119）。这说明 DeerFlow 在逐步收紧"外部数据作为上下文注入"的安全��。
...
### Q: DeerFlow Memory 的 tool mode 和 middleware mode 有什么区别？

> **状态**: verified
> **来源**: discussion / source-code

A: middleware mode（默认）是"被动记忆"：Agent 对话后 MemoryMiddleware 自动提取 facts，Agent 自己不知���记忆了什么。tool mode（新增）是"主动记忆"：Agent 通过 `memory_search`、`memory_add`、`memory_update`、`memory_delete` 四个工具主动管理长期记忆，MemoryMiddleware 不再运行。两种模式互斥。这和 Anthropic 的 Memory Tool（`memory_20250818`）方向一致——从"系统自动替你记"变成"Agent 决定记什么、何时查、何时删"。详见 [memory/tools.py](../../../submodules/deer-flow/backend/packages/harness/deerflow/agents/memory/tools.py)。
