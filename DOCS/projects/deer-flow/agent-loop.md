...
- LangGraph 配置入口：[langgraph.json](../../../deer-flow/backend/langgraph.json)
- Gateway run API：[thread_runs.py](../../../deer-flow/backend/app/gateway/routers/thread_runs.py)
- 创建 run 并启动后台任务：[services.py:356](../../../deer-flow/backend/app/gateway/services.py#L356)
- 后台 worker 主体：[worker.py:121](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py#L121)
- `agent.astream(...)` 驱动位置：[worker.py:305-334](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py#L305-L334)
- lead agent factory：[agent.py:424-428](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L424-L428)
- lead agent 装配：[agent.py:431-562](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L431-L562)
- middleware 组装：[agent.py:270-399](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L270-L399)
- 共享 runtime middleware：[tool_error_handling_middleware.py:128-207](../../../deer-flow/backend/packages/harness/deerflow/agents/middlewares/tool_error_handling_middleware.py#L128-L207)
- 工具集合装配：[tools.py:44-175](../../../deer-flow/backend/packages/harness/deerflow/tools/tools.py#L44-L175)
- ThreadState schema：[thread_state.py:149-157](../../../deer-flow/backend/packages/harness/deerflow/agents/thread_state.py#L149-L157)
- 澄清中断 middleware：[clarification_middleware.py:116-155](../../../deer-flow/backend/packages/harness/deerflow/agents/middlewares/clarification_middleware.py#L116-L155)
- RunManager 并发策略：[manager.py:543-629](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/manager.py#L543-L629)
...
Gateway 在 [thread_runs.py](../../../deer-flow/backend/app/gateway/routers/thread_runs.py) 中实现 LangGraph Platform 风格的 runs API：

- `POST /api/threads/{thread_id}/runs`：创建后台 run，立即返回，见 [thread_runs.py:414-419](../../../deer-flow/backend/app/gateway/routers/thread_runs.py#L414-L419)
- `POST /api/threads/{thread_id}/runs/stream`：创建 run 并用 SSE 流式返回事件，见 [thread_runs.py:422-447](../../../deer-flow/backend/app/gateway/routers/thread_runs.py#L422-L447)
- `POST /api/threads/{thread_id}/runs/wait`：创建 run 并阻塞等待最终状态，见 [thread_runs.py:450-474](../../../deer-flow/backend/app/gateway/routers/thread_runs.py#L450-L474)
- `POST /api/threads/{thread_id}/runs/{run_id}/cancel`：取消运行中的 run，见 [thread_runs.py:499-529](../../../deer-flow/backend/app/gateway/routers/thread_runs.py#L499-L529)

这些 endpoint 统一调用 [services.py:356](../../../deer-flow/backend/app/gateway/services.py#L356) 的 `start_run(...)`。

### run 创建：RunManager + background task

`start_run(...)` 的关键步骤是：

1. 调用 `RunManager.create_or_reject(...)` 创建 `RunRecord`，并处理同一 thread 上已有 run 的并发策略，见 [services.py:423-432](../../../deer-flow/backend/app/gateway/services.py#L423-L432)。
2. 解析 agent factory，默认对应 `lead_agent`，见 [services.py:460](../../../deer-flow/backend/app/gateway/services.py#L460)。
3. 归一化 graph input、构造 run config、合并 runtime context，见 [services.py:461-476](../../../deer-flow/backend/app/gateway/services.py#L461-L476)。
4. 用 `asyncio.create_task(...)` 启动后台 `run_agent(...)`，见 [services.py:478-492](../../../deer-flow/backend/app/gateway/services.py#L478-L492)。

`RunManager.create_or_reject(...)` 在 [manager.py:543-629](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/manager.py#L543-L629) 中实现。它支持的并发策略包括：
...
后台 worker 的核心是 [worker.py:121](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py#L121) 的 `run_agent(...)`。
...
最像“loop”的位置是 [worker.py:305-334](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py#L305-L334)：
...
DeerFlow 在 [langgraph.json:7-9](../../../deer-flow/backend/langgraph.json#L7-L9) 注册 graph：
...
`make_lead_agent(...)` 位于 [agent.py:424-428](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L424-L428)，内部进入 `_make_lead_agent(...)`。

`_make_lead_agent(...)` 在 [agent.py:431-562](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L431-L562) 中做几件事：
...
默认 lead agent 的创建位置见 [agent.py:542-562](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L542-L562)。这里是 DeerFlow Agent Loop 的“图装配点”：模型、工具、middleware、state schema 都在这里汇合。
...
1. 工具集合装配：`get_available_tools(...)`，见 [tools.py:44-175](../../../deer-flow/backend/packages/harness/deerflow/tools/tools.py#L44-L175)
...
`ToolErrorHandlingMiddleware` 在 [tool_error_handling_middleware.py:58-125](../../../deer-flow/backend/packages/harness/deerflow/agents/middlewares/tool_error_handling_middleware.py#L58-L125) 中包装工具调用，把工具异常转换成 `ToolMessage`，让 run 可以继续而不是直接崩溃。
...
共享 runtime middleware 由 [tool_error_handling_middleware.py:128-207](../../../deer-flow/backend/packages/harness/deerflow/agents/middlewares/tool_error_handling_middleware.py#L128-L207) 构造，包括：
...
lead-only middleware 由 [agent.py:270-399](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L270-L399) 构造，包括：
...
其中 [agent.py:372-382](../../../deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py#L372-L382) 体现了 DeerFlow 对“循环失控”和“预算”的控制：`LoopDetectionMiddleware` 用于检测重复 tool-call loop，`TokenBudgetMiddleware` 用于限制单次 run 的 token 消耗。

## 状态与终止条件

### ThreadState

DeerFlow 的状态 schema 是 [ThreadState](../../../deer-flow/backend/packages/harness/deerflow/agents/thread_state.py#L149-L157)，扩展了 LangChain `AgentState`，增加：
...
正常情况下，LangGraph agent 内部完成“模型不再请求工具 / 图到达终点”后，`agent.astream(...)` 结束，worker 在 [worker.py:355-362](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py#L355-L362) 将 run 标记为 `success`。

### 用户取消 / interrupt / rollback

用户取消入口在 [thread_runs.py:499-529](../../../deer-flow/backend/app/gateway/routers/thread_runs.py#L499-L529)。`RunManager.cancel(...)` 会设置 `abort_event` 并 cancel task，见 [manager.py:520-540](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/manager.py#L520-L540)。worker 捕获 abort 后会标记 `interrupted` 或执行 rollback，见 [worker.py:336-362](../../../deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py#L336-L362)。

### clarification 中断

`ClarificationMiddleware` 会拦截 `ask_clarification` tool call，把它转换成 `ToolMessage`，然后返回 `Command(goto=END)` 结束当前图执行，见 [clarification_middleware.py:116-155](../../../deer-flow/backend/packages/harness/deerflow/agents/middlewares/clarification_middleware.py#L116-L155)。
