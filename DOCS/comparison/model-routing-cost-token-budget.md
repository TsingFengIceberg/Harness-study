...
> **日期**: 2026-07-09 | **状态**: draft | **涉及版本**: DeerFlow `c9fb9768d476e28de0294ac7a23cab9819b93f83` / Claw-Code `4ea31c1bc91c4e9bcbd67d51c550c01e127e6d0d` / OpenClaw `57db713ff73f8bf177feac882c1fa40531eea282` / Hermes Agent `88a58ff1355eabe468b4dcd4e152a596932632e6` / OpenHands `1869baf49914309f2115cd8f75d5c7a57a92b371` + software-agent-sdk `ebdb4566aca5a9834578ff8067a7aa6d26730cc4` / LiteLLM `e84a19acd566f6ac95ec6346ba603104adea728f`
...
| LiteLLM | [LiteLLM 学习笔记](../projects/litellm/README.md)、[Model Routing / Cost / Token Budget](../projects/litellm/model-routing-cost-budget.md) | LLM Gateway / AI Gateway，OpenAI-compatible proxy，Router strategy，model price map，virtual key / spend tracking / budget manager / guardrails。 |
...
| LiteLLM | [router.py](../../submodules/litellm/litellm/router.py) | Router 主实现，后续重点看 deployment selection、fallback、retry、pre-call checks 与 response metadata。 |
| LiteLLM | [simple_shuffle.py](../../submodules/litellm/litellm/router_strategy/simple_shuffle.py) | 默认随机 / 加权随机路由策略入口。 |
| LiteLLM | [least_busy.py](../../submodules/litellm/litellm/router_strategy/least_busy.py) | 当前 in-flight 请求数最少优先的并发均衡策略入口。 |
| LiteLLM | [lowest_latency.py](../../submodules/litellm/litellm/router_strategy/lowest_latency.py) | 基于近期 latency / time-to-first-token 样本选择 deployment 的低延迟策略入口。 |
| LiteLLM | [lowest_tpm_rpm_v2.py](../../submodules/litellm/litellm/router_strategy/lowest_tpm_rpm_v2.py) | 按当前分钟 TPM / RPM 使用量和限额选择 deployment 的限流友好策略入口。 |
| LiteLLM | [lowest_cost.py](../../submodules/litellm/litellm/router_strategy/lowest_cost.py) | 成本优先路由策略入口，适合补充 model routing / cost routing 视角。 |
| LiteLLM | [budget_limiter.py](../../submodules/litellm/litellm/router_strategy/budget_limiter.py) | 预算限制型路由策略入口，后续核验请求级 / 模型级预算防线。 |
| LiteLLM | [cooldown_handlers.py](../../submodules/litellm/litellm/router_utils/cooldown_handlers.py) | 近期失败 deployment 的 cooldown 判断和候选过滤辅助逻辑。 |
| LiteLLM | [user_api_key_auth.py](../../submodules/litellm/litellm/proxy/auth/user_api_key_auth.py) | Proxy key auth 入口，包含 key 模型权限和 fallback 目标校验。 |
| LiteLLM | [proxy_server.py](../../submodules/litellm/litellm/proxy/proxy_server.py) | OpenAI-compatible proxy server 主入口之一，承载 HTTP API、管理端点和请求生命周期。 |
| LiteLLM | [budget_manager.py](../../submodules/litellm/litellm/budget_manager.py) | Budget 管理入口，连接 spend 更新、预算检查和预算重置。 |
| LiteLLM | [cost_calculator.py](../../submodules/litellm/litellm/cost_calculator.py) | 模型调用成本计算入口，连接 token usage、model price map 和 spend tracking。 |
| LiteLLM | [model_prices_and_context_window.json](../../submodules/litellm/model_prices_and_context_window.json) | 模型价格、上下文窗口和能力元数据表，是模型网关层成本治理的关键资料。 |
...
## 九、LiteLLM 与 DeerFlow：模型网关机制边界与接入设想

> **本节记录讨论结论**：LiteLLM 更像“模型调度与财务闸机”，DeerFlow 更像“Agent 运行时与任务工厂”。两者不在同一层，最自然的接入方式不是让 LiteLLM 替代 DeerFlow Gateway，而是让 LiteLLM 替代 DeerFlow 中“直连各家模型 provider 的出口层”。

### 9.1 先分清两个 Gateway：Agent run 网关 vs 模型网关

DeerFlow 的 Gateway 主要管理一次 Agent run 的生命周期：接收前端 / 外部请求，创建 run / thread / runtime context，解析 `model_name`、`thinking_enabled`、`reasoning_effort` 等可配置项，再启动 lead agent / subagent / tools / sandbox / middleware 链。相关入口见 [services.py](../../submodules/deer-flow/backend/app/gateway/services.py)、[worker.py](../../submodules/deer-flow/backend/packages/harness/deerflow/runtime/runs/worker.py) 和 [lead_agent/agent.py](../../submodules/deer-flow/backend/packages/harness/deerflow/agents/lead_agent/agent.py)。

LiteLLM 的 Gateway / Proxy 主要管理模型调用：校验 virtual key / team / budget / model allowlist，在 model group 背后的多个 deployment 之间做 health / cooldown / latency / cost / TPM-RPM / fallback 调度，然后把 usage、latency、spend、failure 等指标写回网关状态。相关入口见 [router.py](../../submodules/litellm/litellm/router.py)、[user_api_key_auth.py](../../submodules/litellm/litellm/proxy/auth/user_api_key_auth.py) 和 [budget_limiter.py](../../submodules/litellm/litellm/router_strategy/budget_limiter.py)。

两者组合后更合理的调用链是：

```text
用户 / 前端
  -> DeerFlow Gateway：管任务、会话、Agent、工具、沙箱、上下文
    -> LiteLLM Gateway：管模型、provider、key、预算、fallback、路由
      -> OpenAI / Anthropic / Gemini / Doubao / Ollama / Azure ...
```

**精髓标记：DeerFlow Gateway 管“这个 Agent run 怎么跑”；LiteLLM Gateway 管“这一次 LLM call 到底打到哪个模型部署上”。**

### 9.2 机制对比表

| 维度 | DeerFlow | LiteLLM | 组合后的合理分工 |
|---|---|---|---|
| 模型定义 | `ModelConfig`：`name`、`use`、`model`、`api_key`、`base_url`、capability、pricing。见 [model_config.py](../../submodules/deer-flow/backend/packages/harness/deerflow/config/model_config.py)。 | `model_list` / model group / deployment：一个对外模型名背后可挂多个 provider、key、region、fallback。 | DeerFlow 暴露 Agent 视角的逻辑模型，如 `default` / `fast` / `deep` / `vision`；LiteLLM 管真实 provider deployment 池。 |
| 模型选择 | run context `model_name` -> agent config -> 全局默认模型；再由 [factory.py](../../submodules/deer-flow/backend/packages/harness/deerflow/models/factory.py) 实例化 ChatModel。 | 请求 model group 后，在候选 deployments 里过滤 unhealthy / cooldown / 超预算 / 超限流 / tag 不匹配，再按策略选一个。 | DeerFlow 决定“这个 agent 要哪个逻辑模型”；LiteLLM 决定“这个逻辑模型本次落到哪个 deployment”。 |
| 预算控制 | [TokenBudgetMiddleware](../../submodules/deer-flow/backend/packages/harness/deerflow/agents/middlewares/token_budget_middleware.py) 统计单次 run token，warn / hard stop，避免 agent loop 失控。 | virtual key / user / team / org / model / provider / deployment / tag budget，TPM / RPM，budget limiter 和 spend tracking。 | LiteLLM 管组织级花钱闸门；DeerFlow 管单次任务 token 刹车。 |
| 权限边界 | Gateway 白名单合并 context key，并校验 model allowlist；scheduler / non-interactive 等内部 context 不能由普通 client 随便伪造。 | virtual key 限制 allowed models / routes / budget / rate limit；fallback 目标也要过 key 模型权限校验。 | DeerFlow 管业务入口授权；LiteLLM 管模型资源授权。 |
| fallback / health | 更关注 Agent runtime 错误处理、tool error、LLM error、safety finish、token stop。 | cooldown、background health check、retry、same-group fallback、cross-model fallback、context-window fallback、content-policy fallback。 | DeerFlow 保证 agent 失败后体面收口；LiteLLM 保证底层 provider / deployment 出问题时自动换线路。 |
| 成本观测 | run / middleware / tracing 侧看到 token 使用和 stop reason，便于解释“本次 agent 为什么停”。 | cost calculator、spend logs、virtual key / team / provider spend，便于对账和预算治理。 | DeerFlow 负责 run 级归因；LiteLLM 作为 provider spend 和预算的权威来源。 |

**精髓标记：DeerFlow 的模型配置像“Agent 工位需要什么脑子”；LiteLLM 的模型配置像“这个脑子背后有哪些供应商线路、价格、限额和故障预案”。**

### 9.3 预算差异：单次 run 刹车 vs 组织级花钱闸门

DeerFlow 的 token budget 和 LiteLLM 的 budget 不应该互相替代，而应该叠加：

```text
LiteLLM budget：
  调用前 / 网关层
  控制谁有资格继续花钱、打哪个 provider、是否 fallback

DeerFlow token budget：
  Agent run 内部
  控制这个任务是否该继续思考、继续调用工具、继续展开
```

DeerFlow 的 [TokenBudgetMiddleware](../../submodules/deer-flow/backend/packages/harness/deerflow/agents/middlewares/token_budget_middleware.py) 更像“这辆车这趟别开过头”：它统计单次 run 的 token，接近阈值时提醒，硬阈值时阻止继续扩张 tool call / agent loop。

LiteLLM 的 budget 更像“整个车队今天还能烧多少油”：它可以在 key、user、team、org、project、provider、deployment、tag 等层面控制 spend / TPM / RPM，并在候选池过滤阶段排除超预算 deployment。

**精髓标记：DeerFlow 的 token budget 是“本次任务刹车”；LiteLLM 的 budget 是“模型资源财务闸门”。**

### 9.4 推荐接入方式：优先用 LiteLLM Proxy，不急着嵌 SDK

如果改装 DeerFlow，让 DeerFlow 的模型配置通过 LiteLLM 实现，推荐优先走 **LiteLLM Proxy + OpenAI-compatible endpoint**：

```text
DeerFlow Agent Runtime
  -> LangChain ChatOpenAI / OpenAI-compatible client
    -> base_url = LiteLLM Proxy /v1
    -> model = df-default / df-fast / df-deep / df-vision
    -> api_key = LiteLLM virtual key
      -> LiteLLM Router
        -> real provider deployments
```

DeerFlow 的 [factory.py](../../submodules/deer-flow/backend/packages/harness/deerflow/models/factory.py) 已经支持通过 `use` 动态实例化 ChatModel，并把 `base_url` / `api_key` / `model` 等字段传给 provider client。因此最小接入可以不重写 Agent runtime，只需要把 DeerFlow 里的真实 provider 配置改成指向 LiteLLM 的逻辑模型组。
...
models:
  - name: default
    display_name: Default via LiteLLM
...
不建议一开始就把 LiteLLM Router SDK 嵌入 DeerFlow 进程，因为 LiteLLM 最有价值的能力是中心化治理：virtual key、team budget、spend tracking、health / cooldown、fallback、provider key 隔离和 OpenAI-compatible proxy。嵌 SDK 可能少一个网络 hop，但会让多实例状态、权限、预算和运维边界变复杂。

### 9.5 迁移分阶段方案

1. **Phase 1：最小可用接入**
   - 部署 LiteLLM Proxy。
   - 在 LiteLLM 配置 `df-default`、`df-fast`、`df-deep`、`df-vision` 等 model group。
   - DeerFlow `models` 统一改成 OpenAI-compatible client，`base_url` 指向 LiteLLM。
   - 先验证 lead agent、subagent、streaming、tool calling、usage metadata 是否正常。

2. **Phase 2：权限和预算接入**
   - DeerFlow Gateway 根据用户 / team / workspace 选择对应 LiteLLM virtual key，避免所有请求共用 admin key。
   - 对齐 DeerFlow `model_name` allowlist 与 LiteLLM key-level model allowlist。
   - LiteLLM 配 key / team budget、TPM / RPM；DeerFlow 继续保留 per-run TokenBudgetMiddleware。

3. **Phase 3：路由和 fallback 接入**
   - 给 `df-default` 等模型组配置多个 deployment。
   - 按用途选择策略：`df-fast` 可偏 cost-based / simple-shuffle，`df-default` 可用 latency-based / usage-based，`df-deep` 更重质量与稳定 fallback。
   - 开启 cooldown / health check / fallback，但每个 model group 内 fallback 必须能力等价。

4. **Phase 4：观测和成本对账**
   - DeerFlow 调 LiteLLM 时传递 `run_id`、`thread_id`、`user_id`、`agent_name`、`subagent_name`、`mode`、`workspace` 等 metadata / tags。
   - DeerFlow 继续做 run 级 token / stop reason / tracing 归因。
   - LiteLLM 作为 provider spend、预算和账单的权威来源。

### 9.6 接入风险和边界条件

最需要注意的是能力声明与实际 fallback 的一致性。DeerFlow 会根据 `supports_vision`、`supports_thinking`、`supports_reasoning_effort` 等模型能力构建 runtime 行为；如果 LiteLLM 底层 fallback 到能力更弱的 deployment，DeerFlow 可能不知道。因此 model group 应按能力分组，例如 `df-text`、`df-vision`、`df-reasoning`、`df-long-context`、`df-local`，并要求组内 fallback 能力不低于组名承诺。

第二个风险是 usage metadata。DeerFlow 的 TokenBudgetMiddleware 依赖模型响应中的 usage 信息；接入 LiteLLM 后必须验证普通调用、streaming 调用、tool calling、fallback 后响应是否都能带回足够准确的 usage。

第三个风险是 tool calling 格式。DeerFlow 的 LangGraph / LangChain agent loop 对 tool call / tool result 格式敏感；LiteLLM 虽然提供 OpenAI-compatible 接口，但不同 provider 的 tool schema 转换、parallel tool call、tool result replay 都需要实测。

第四个风险是 key 边界。DeerFlow 前端 / 外部 client 不应该直接拿 LiteLLM admin key；更合理的是 client 只持有 DeerFlow 业务 token，DeerFlow 服务端根据用户 / team 选择受限的 LiteLLM virtual key。后台 scheduler / non-interactive run 也应使用独立 key 和预算，避免定时任务误配置后打爆普通用户预算。

**最终精髓：DeerFlow 不需要学会所有 provider 路由策略；它只需要把“模型调用出口”交给 LiteLLM。DeerFlow 继续专注 Agent Harness，LiteLLM 专注模型网关。**
