...
### Q: Coze Studio 和 Dify 应该如何做第一轮平台定位比较？

> **状态**: to-verify
> **来源**: official-docs / source-code / inference

A: 这是一条“同层平台、不同中心抽象”的初步比较，不是功能清单或优劣排名。两者都把模型接入、Agent、可视化 Workflow、知识 / RAG 相关能力、工具 / 扩展和 API 交付带到同一应用平台层。Coze Studio 的公开资源模型更显式地围绕 Agent / App 以及 Workflow、Plugin、Knowledge Base、Database、Prompt 等资源组织；Dify 的公开代码结构则将 Application、Dataset / RAG、Workflow、Agent、Tool / Plugin / MCP 分为清晰的核心区域。因而当前可先理解为：Coze Studio 更适合从“构建并发布一个可使用资源的 Agent / App”进入，Dify 更适合从“构建、交付和运营一个带知识能力的 LLM 应用”进入；两条路径重叠很大，不能据此推断任何一方没有另一方能力。与 Coze Studio 相关但独立的 [CozeLoop](../projects/cozeloop/README.md) 则把 Prompt、评测、实验和 Trace 观测放在单独的 AgentOps 生命周期层。

本仓库目前只建立了 [Dify](../projects/dify/README.md)、[Coze Studio](../projects/coze-studio/README.md) 和 [CozeLoop](../projects/cozeloop/README.md) 的源码入口。RAG 细节、插件 / MCP、Workflow runtime、权限和 AgentOps 是否内聚等问题，需要后续按同一源码粒度继续核验。
...
A: DeerFlow 有受控的经验沉淀与能力扩展，而不是完整的自动强化学习。当前 run 内，Goal Evaluator 会检查目标是否完成并让 Agent 在满足条件时继续；跨会话，MemoryMiddleware / MemoryUpdater 能把用户偏好和显式纠正写入长期 memory；可选的 Skill Evolution 允许 Agent 通过受控 `skill_manage` 将复杂或已修正的流程沉淀为 custom Skill；Custom Agent 也能由用户或管理员持久化配置。当前未见基于 feedback 自动改全局 prompt、模型路由、工具策略或模型权重的闭环。详见 [DeerFlow 自进化、Skill Evolution 与 Custom Agent 配置](../projects/deer-flow/self-evolution-custom-agents.md)。
...
A: 当前没有独立的历史任务聚类或 workflow mining 模块。开启 `skill_evolution.enabled` 后，DeerFlow 通过 system prompt 告诉模型在“任务使用 5+ 工具调用、克服非显然错误、用户纠正且修正有效、发现非简单的重复工作流、现有 Skill 有缺口”等情形下考虑创建或更新 Skill。是否沉淀由 LLM 根据当前对话、工具调用和结果做语义判断；创建前 prompt 要求优先确认用户，写入必须经 `skill_manage` 的校验和安全扫描。因此这是提示词约束的机会式能力沉淀，而不是统计驱动的自动经验挖掘。详见 [DeerFlow 自进化、Skill Evolution 与 Custom Agent 配置](../projects/deer-flow/self-evolution-custom-agents.md#四skill-evolution受控的能力沉淀而不是自动流程挖掘)。
...
A: 不必。Custom Agent 可由用户 / operator 直接维护其 per-user `config.yaml` 和 `SOUL.md`，显式定义身份、行为准则、默认模型、工具组和 Skill 白名单；也可启用 `agents_api.enabled=true` 后，通过 `GET/POST/PUT /api/agents` 管理。对话中的 `update_agent` 只是当前 custom agent 在用户明确要求时使用的便利入口，并非唯一或推荐的生产配置路径。详见 [DeerFlow 自进化、Skill Evolution 与 Custom Agent 配置](../projects/deer-flow/self-evolution-custom-agents.md#五custom-agent既可以对话更新也可以显式配置)。
...
### Q: DeerFlow Memory 的 tool mode 是否改变了自进化的评估？

> **状态**: verified
> **来源**: discussion / source-code

A: 是的。之前 DeerFlow 只有 middleware mode——Agent 被动地让系统提取和注入记忆。新增 tool mode 后，Agent 可以通过 `memory_search`、`memory_add`、`memory_update`、`memory_delete` 主动管理长期记忆。这提升了"自进化"的自主性，但仍属"受控的经验沉淀"而非自动 RL——Agent 的决定仍然在配置边界内，且不改变全局 prompt、模型路由或模型权重。详�� [DeerFlow Memory Tool Mode](../projects/deer-flow/self-evolution-custom-agents.md#23-memory-tool-modeagent-主动管理自己的长期记忆v20-新增)。


### Q: MCP 的 Tools、Resources、Prompts 三种能力面分别解决什么问题？

> **状态**: verified
> **来源**: discussion / official-docs / source-code

A: Tool 是 Agent 可执行的动作（搜索、创建、删除、查询、部署），���品闭环最短、Host 支持最广。Resource 是可读取的外部数据对象（schema、配置、文档全文），让 Agent 在执行动作前先了解环境。Prompt 是 Server 提供的标准任务模板（审查清单、报��框架），由后端团队预设"这件事的标准做法"。详见 [MCP 基础概念](../concepts/mcp.md#二mcp-能暴露什么深度理解-toolsresourcesprompts)。

### Q: MCP 工具调用的安全应该怎么层层拆解？

> **状态**: verified
> **来源**: discussion / source-code

A: 不应只用一句"不要信���外部输入"覆��。本文讨论形成了七层防线模型：Layer 0 管哪些 server 可以连接（server trust），Layer 1 管哪些 tool schema 能暴露给模型（tool visibility），Layer 2 管当前任务/意图边界下能不能调这个 tool（intent/task boundary），Layer 3 管是否需要人类审批（execution approval / HITL），Layer 4 管 server 和下游系统的凭据权限（server-side permission），Layer 5 管 tool result 的脱敏/���算/防注入（result handling），Layer 6 管事后追溯和回滚（audit & recovery）。详见 [MCP 基础概念](../concepts/mcp.md#七安全mcp-工具调用的七层防线模型)。

### Q: MCP 标准化的是什么，不自动完成的是什么？

> **状态**: verified
> **来源**: discussion

A: MCP 标准化的是连接面（Host 如何发现 server capabilities、如何建立会话、如何传输调用和结果），属于七层防线中 Layer 0-1 的协议部分。它不会自动���成：server 信任审核、工具可见性策略、任务级授权、人类审批、server 凭据管理、tool result 脱敏/注入防护、以及端到端审计和回滚。详见 [MCP 基础概念](../concepts/mcp.md#七层防线的精髓)。

### Q: DeerFlow 在 MCP 七层防线上覆盖了什么？

> **状态**: verified
> **来源**: discussion / source-code

A: DeerFlow 各���对应：Layer 0（`extensions_config.json` + MCP client builder）、Layer 1（`get_available_tools(...)` → skill filter → deferred tool search）、Layer 2（GuardrailMiddleware deny/allow 前策略检查���、Layer 3（GuardrailMiddleware + ClarificationMiddleware，但非 per-tool approval UI）、Layer 4（MCP tools/cache 协议面，凭据管理在外部层）、Layer 5（ToolOutputBudget、SandboxAudit、ToolErrorHandling）、Layer 6（Gateway run lifecycle + RunManager cancel/rollback）。完整的凭据管理、per-tool-call 审计和端到端回���需要 Host、server 和下游系��三方协同。详见 [MCP 基础概念](../concepts/mcp.md#七安全mcp-工具调用���七层防线模型) 末尾的 DeerFlow 防线映射表。

### Q: Lark CLI 和官方 Lark OpenAPI MCP 交给 Agent 使用时，有什么区别、共同点和联系？

> **状态**: verified
> **来源**: official-docs / source-code

A: 两者都把飞书 / Lark 的业务能力交给 Agent 使用，但**接入边界不同**。官方 [lark-cli](https://github.com/larksuite/cli) 是面向人、脚本和 Agent Skills 的命令行客户端；官方 [Lark OpenAPI MCP](https://github.com/larksuite/lark-openapi-mcp) 是面向 MCP Host 的协议 Server，当前 README 标为 Beta。它们最终都调用 Lark / 飞书 OpenAPI，因此不能把它们理解为两套独立权限系统。

| 维度 | `lark-cli` | `lark-openapi-mcp` |
|---|---|---|
| 主要边界 | shell / terminal 命令面；人和 Agent 都可直接使用 | MCP Server；由支持 MCP 的 Host 发现并调用工具 |
| Agent 怎样接入 | Harness 先提供受控 shell 工具，Agent 再运行命令、读取 JSON / 表格等结构化输出 | Host 连接 Server、发现允许的 tool schema、交给模型选择并把 tool result 回填上下文 |
| 能力组织 | shortcut、平台同步的 API command、raw API 三层；README 当前列出 200+ 命令和 20+ Agent Skills | common API、preset tool collection 或指定 tools；不是假定所有 OpenAPI 都自动等价暴露 |
| 适合场景 | 本地脚本组合、人工排障、显式 `--dry-run` 预览、已有终端型 Agent / Skill 的工作流 | 已有 MCP Host、希望统一 server trust / tool discovery / tool visibility / 调用回写的 Agent 产品 |
| 身份与权限 | app 配置、用户或 bot 身份、OAuth scope / token；CLI 本身不绕过 OpenAPI 授权 | app 或 user OAuth、scope 与 tool preset 配置；MCP 本身不绕过 OpenAPI 授权 |

**共同点**是：都需要正确的应用配置、身份选择和最小 OAuth scope；都可以让 Agent 读写 Docs、Base、Calendar、IM、Tasks 等企业协作对象；写入、发送、审批、批量操作仍应由 Host / Agent runtime 的任务边界和人类确认策略治理。覆盖范围和兼容性随版本、认证身份和启用的工具集合变化，不能假定二者完全等价。

**联系**是：MCP 可以看作把一组 Lark OpenAPI 能力包装为 Host 原生可发现的工具；CLI 则把同一类业务能力包装为可组合命令。一个 terminal-capable Agent 可以直接调用 CLI；一个 MCP-native Host 可以直接接 MCP Server；两者也可以共存，甚至把 CLI 再封装成 MCP 后端。无论哪条路径，均应套用 [MCP 七层安全防线](../concepts/mcp.md#七安全mcp-工具调用的七层防线模型)：审查二进制 / Server 来源（Layer 0）、限制可见 command / tool（Layer 1）、收窄任务和 OAuth scope（Layer 2 / 4）、为高影响写操作设置确认（Layer 3），并处理输出中的敏感数据、提示注入和审计（Layer 5 / 6）。

**选型口诀**：需要“在本地把飞书能力当命令和 Skill 组合起来”时优先 CLI；需要“让 MCP Host 统一发现、筛选、调用和回填飞书工具”时优先 MCP。差异是运行时适配面，不是权限豁免或安全替代。
