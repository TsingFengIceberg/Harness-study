...
A: 工具执行在执行面。App Server / Agent Canvas 控制面负责创建 conversation、管理 sandbox、准备 tools / MCP / skills / hooks / secrets / workspace 配置、转发消息、查询状态和展示事件；真正执行工具的是 Agent Server / SDK / `openhands-tools`，在对应 workspace / sandbox 中运行。可以记成：控制面决定“这个 conversation 有哪些工具、在哪个 sandbox 跑”；执行面负责“模型什么时候调用工具、工具实际怎么跑、结果怎么回给模型”。当前仓库 [pyproject.toml](../../openhands/pyproject.toml) 依赖 `openhands-agent-server`、`openhands-sdk`、`openhands-tools`，也体现了这个执行面边界。
