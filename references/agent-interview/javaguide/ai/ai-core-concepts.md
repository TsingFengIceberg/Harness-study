...
> **来源**: [https://javaguide.cn/ai/ai-core-concepts.html](https://javaguide.cn/ai/ai-core-concepts.html)  
> **抓取日期**: 2026-07-30  
> **资料性质**: JavaGuide 第三方网页机械提取  
> **证据状态**: to-verify

[返回 JavaGuide 本地索引](../README.md)

---

[![JavaGuide 官方知识星球](https://oss.javaguide.cn/xingqiu/xingqiu.png)](https://javaguide.cn/about-the-author/zhishixingqiu-two-years.html)这篇文章只做原文摘录和概念归类，不重新改写已有解释。每个二级模块下面都整理了相关原文链接，想深入看完整上下文，可以点进原文继续读。
...
- [大模型提示词工程（Prompt Engineering）是什么？提示词技巧有哪些？](https://javaguide.cn/ai/agent/prompt-engineering.html)
...
- [AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册](https://javaguide.cn/ai/agent/agent-basis.html)
- [AI 工作流中的 Workflow、Graph 与 Loop：从概念到实现](https://javaguide.cn/ai/agent/workflow-graph-loop.html)
- [上下文工程(Context Engineering) 是什么？和 Prompt Engineering 有什么区别？](https://javaguide.cn/ai/agent/context-engineering.html)
- [AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制](https://javaguide.cn/ai/agent/agent-memory.html)
- [什么是 Model Context Protocol (MCP)？和 Function Calling、Agent 什么关系？](https://javaguide.cn/ai/agent/mcp.html)
- [Agent Skills 是什么？和 Prompt、MCP 到底差在哪？](https://javaguide.cn/ai/agent/skills.html)
- [一文搞懂 Harness Engineering：六层架构、上下文管理与一线团队实战](https://javaguide.cn/ai/agent/harness-engineering.html)
- [Loop Engineering 是什么？为什么说它是新瓶装旧酒？](https://javaguide.cn/ai/agent/loop-engineering.html)
...
- [RAG 文档处理与切分策略：从解析、清洗、Chunking 到多模态内容处理](https://javaguide.cn/ai/rag/rag-document-processing.html)
- [万字详解 RAG 优化：从召回、重排到上下文工程的系统调优](https://javaguide.cn/ai/rag/rag-optimization.html)
- [万字详解 GraphRAG：为什么只靠向量检索撑不起复杂知识问答](https://javaguide.cn/ai/rag/graphrag.html)
...
4. 文档拆分（Chunking）：用文本分割器把文档切成较小片段。这一步要兼顾语义完整性、Embedding 模型输入长度、生成模型上下文窗口和召回粒度。Chunk 太大容易引入噪声，太小又可能丢上下文。拆分策略会直接影响召回质量，详细可以看 [RAG 文档处理篇](https://javaguide.cn/ai/rag/rag-document-processing.html)。
...
6. 存储到向量存储或索引系统：把嵌入向量、原始内容和对应元数据存入向量存储或向量索引系统，比如 Milvus、pgvector、Elasticsearch / OpenSearch 向量检索，或基于 Faiss 构建本地向量索引。向量数据库选型、索引算法和 pgvector 实践可以看 [RAG 向量库篇](https://javaguide.cn/ai/rag/rag-vector-store.html)。
